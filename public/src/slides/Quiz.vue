    ...
    <section ref="section">
        <template v-if="!started">
            <div class="h-[400px] grid place-content-center">
                <h2>Quiz</h2>
                <button @click="started = true" class="bg-gray-800 bg-opacity-50 border border-gray-800 px-4 py-2 text-base rounded-lg">Start Quiz</button>
            </div>
        </template>
        <template v-if="started">
            <div class="h-[400px] grid place-content-center" v-if="data.questions.length > activeQuestion">
                <h6 class="text-4xl !mb-10">{{ data.questions[activeQuestion].frage }}</h6>
                <form class="grid grid-cols-2 px-20 gap-4">
                    <template v-for="(question, i) in data.questions[activeQuestion].optionen">
                        <div>
                            <label
                                ...
                                <input @change="showStatus ? '' : HandleChange(question.id, data.questions[activeQuestion].id)"
                                    class="hidden" type="radio" :value="question.id"
                                    :name="data.questions[activeQuestion].id.toString()">
            ...
            <div v-else class="h-[400px] grid place-content-center">
                <h2>Ergebnis</h2>
                <p>Du hast {{ result }} von {{ data.questions.length }} Fragen richtig beantwortet!</p>
            </div>
        </template>
...
function HandleAnswer() {
    ...
    if (currentQuestion.value.selected == data.questions[activeQuestion.value].antwort) {
    ...
    setTimeout(() => {
        ...
        if(data.questions.length == activeQuestion.value) {
            poof();
            return;
        }
    }, 1000)
